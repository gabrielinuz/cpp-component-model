 ...
 * @brief Servidor HTTP concurrente multi-verbo con gestion dinamica de memoria RAII.
 */
...
class HttpServer 
{
    private:
        int port_;
        /// Mapa anidado: [VERBO][PATH] -> Handler (Ej: ["POST"]["/api/upload"] -> lambda)
        std::unordered_map<std::string, std::unordered_map<std::string, RouteHandler>> routes_;
        std::mutex console_mutex_;

        /**
        * @brief Lee secuencialmente el socket garantizando la integridad de payloads grandes (archivos).
        */
    ...
    public:
        ...
        /**
        * @brief Registro generico de rutas asociadas a un verbo HTTP especifico.
        */
        ...
        /// Fachadas semanticas (Sugar Syntax) para diseño de APIs limpias
        void get(const std::string& path, RouteHandler h) { add_route("GET", path, h); }
        void post(const std::string& path, RouteHandler h) { add_route("POST", path, h); }
        void put(const std::string& path, RouteHandler h) { add_route("PUT", path, h); }
        void del(const std::string& path, RouteHandler h) { add_route("DELETE", path, h); }
        void patch(const std::string& path, RouteHandler h) { add_route("PATCH", path, h); }

        void serve_static(const std::string& route, const std::string& filepath, const std::string& content_type) 
        {
        ...
        void listen_and_serve() 
        {
            ...
            if (server_fd == -1) throw std::runtime_error("Error de socket");
            ...
            if (bind(server_socket.get(), (struct sockaddr*)&address, sizeof(address)) < 0) throw std::runtime_error("Error bind");
            if (listen(server_socket.get(), 10) < 0) throw std::runtime_error("Error listen");

            std::cout << "Servidor REST con soporte de archivos activo en puerto " << port_ << "\n";

            while (true) 
            {
                struct sockaddr_in client_addr{};
                socklen_t len = sizeof(client_addr);
                int client_fd = accept(server_socket.get(), (struct sockaddr*)&client_addr, &len);
                if (client_fd >= 0) 
                {
                    std::thread(&HttpServer::process_client, this, client_fd).detach();
                }
            }
        }
};
